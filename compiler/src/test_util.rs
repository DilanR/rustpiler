...
macro_rules! test_expr {
    ( $expr:expr ) => {{
        ...
        let expr: compiler::ast::Expr = compiler::parse::parse(src);
        let result: compiler::ast::Literal =
            compiler::common::Eval::<compiler::vm::Val>::eval(&expr)
                .unwrap()
                .into();
        assert_eq!(result, compiler::ast::Literal::from(expected));
    }};
}
    ...
    ( $block:block $(, $expected:expr )? ) => {
        {
            ...
            let block: Block = compiler::parse::parse(src);
            compiler::test_util::assert_value(&block, result);
            compiler::test_util::test_display_parse(block);  // NOTE: comment out to disable Display testing
        }
    };
    ( $tests:block, $( $code:stmt ),+; $x:expr ) => {
        {
            ...
            compiler::test_util::assert_eval(&block, expected);
...
// New helpers such to test pipeline.

pub fn assert_parse_type<T>(src: &str, expected: Type)
where
    T: syn::parse::Parse + std::fmt::Display + Eval<Type>,
{
    let p: T = parse(src);
    assert_type(&p, expected);
}

pub fn assert_parse_type_fail<T>(src: &str)
where
    T: syn::parse::Parse + std::fmt::Display + Eval<Type>,
{
    let p: T = parse(src);
...
pub fn assert_parse_value<T>(src: &str, expected: impl Into<Val>)
where
    T: syn::parse::Parse + std::fmt::Display + Eval<Val>,
{
    let p: T = parse(src);
    assert_value(&p, expected.into());
}

pub fn assert_parse_eval<T>(src: &str, expected: impl Into<Val> + Into<Type> + Copy)
where
    T: syn::parse::Parse + std::fmt::Display + Eval<Type> + Eval<Val>,
{
    let p: T = parse(src);
    assert_eval(&p, expected);
}
